...
class DataPersister:
    ...
    def recDataPersist(data_dict):
        ...
        if status == Constants.STR_SUCCESS:
            ...
            DataPersister.writeDictDataInExcelModelMontlyBasket(data_dict,fileName,config_dict[Constants.STRING_APP]['daily_excel_sheet_one_name'].replace(Constants.UNDERSCORE,Constants.SPACE))

    def writeDictDataInExcelModelMontlyBasket(data_dict,file_name,sheet_name):
        #print(data_dict)
        row_counter=3
        for item in DataPersister.items_persist_in_excel:
            #print(data_dict[item])
            row_counter=DataPersister.persist_item_data_in_excel(item,row_counter,data_dict[item],file_name,sheet_name)
            row_counter+=1
    ...
    def persist_item_data_in_excel(item,row_counter,data,file_name,sheet_name):
        ...
        for d in data:
            ...
            #print(d)
            for v in DataPersister.vendor_name_list[Constants.NUM_3:len(DataPersister.vendor_name_list)]:
                #print(v)
                price_counter=0
    ...
    def firt2RowPersistInSheet(file_name,sheet_name):
        ...
        #print(indx_no)
        workbook.active = indx_no
        DataPersister.writeInitialData(file_name,workbook)
    ...
    def writeInitialData(file_name,workbook):
        ...
        for col_range in range(Constants.NUM_1, len(DataPersister.vendor_name_list)+Constants.NUM_1):
